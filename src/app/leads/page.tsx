...
import { CountPill, PageHeader } from '../../components/recruitment-ui';
...
export default function RecruitmentAgencyDashboard() {
  ...
  return (
    <div className="recruitment-page overflow-hidden">
      <main className="relative flex flex-col overflow-hidden">
        <div className="recruitment-page-shell max-w-[1440px]">
          <PageHeader
            title="Leads"
            description="Track, qualify, and convert potential clients into active hiring partners without disturbing the existing lead workflows."
            meta={<CountPill>{filteredLeads.length} visible</CountPill>}
            actions={
              <>
          ...
          <div className="recruitment-table-shell">
            <div className="recruitment-toolbar rounded-none rounded-t-2xl border-x-0 border-t-0 shadow-none">
              <div className="recruitment-search-shell w-full md:max-w-md">
                <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-slate-400" size={18} />
                <input 
                  type="text" 
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="recruitment-muted-chip">
                  <Filter size={14} className="text-slate-400" />
                  <span className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">Filters</span>
                </div>
                
                <div className="flex items-center gap-2">
                  <select 
                    className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-500/10"
                    value={statusFilter}
                  ...
                  <select className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-500/10">
                  ...
                  <button 
                    className="recruitment-secondary-button h-11 px-3 text-sm text-slate-500 hover:text-red-600"
                    onClick={() => {
                      setSearchQuery('');
                      setStatusFilter('All');
                    }}
            ...
            <div className="overflow-x-auto">
              {loading && (
                <div className="px-6 py-12 text-center text-sm font-medium text-slate-500">Loading leads...</div>
              )}
              {error && !loading && (
                <div className="px-6 py-12 text-center text-sm font-medium text-red-500">Error: {error}</div>
              )}
              {!loading && !error && (
                <table className="w-full text-left">
                  <thead>
                    <tr className="recruitment-table-head-row">
                      <th className="recruitment-table-head-cell px-6">Lead / Company</th>
                      <th className="recruitment-table-head-cell px-6">Source</th>
                      <th className="recruitment-table-head-cell px-6">Contact</th>
                      <th className="recruitment-table-head-cell px-6">Status</th>
                      <th className="recruitment-table-head-cell px-6">Assigned To</th>
                      <th className="recruitment-table-head-cell px-6">Last Follow-up</th>
                      <th className="recruitment-table-head-cell px-6 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    ...
                    <tr 
                      key={lead.id} 
                      className={`group cursor-pointer transition-colors hover:bg-slate-50 ${selectedLeadId === lead.id ? 'bg-blue-50/50' : ''}`}
                    ...
                    >
                      ...
                      <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                        <div className="flex flex-col gap-2">
                          <select
                            className="h-9 rounded-full border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            value={lead.status}
                            onChange={(e) =>
                          ...
                          {statusEdit.leadId === lead.id && (
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                className="flex-1 rounded-lg border border-slate-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                      ...
                      <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 shadow-sm hover:text-blue-600 hover:bg-blue-50" title="View Details" onClick={() => setSelectedLeadId(lead.id)}>
                            <Eye size={18} />
                          </button>
                          <button 
                            className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 shadow-sm hover:text-green-600 hover:bg-green-50" 
                          ...
                          <button 
                            className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 shadow-sm hover:text-red-600 hover:bg-red-50" 
                            title="Delete Lead"
                            onClick={async (e) => {
                              e.stopPropagation();
...
const SummaryCard = ({ label, count, color, icon }: { label: string, count: number, color: string, icon: React.ReactNode }) => {
  ...
  return (
    <div className={`recruitment-kpi-card ${s.border}`}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className={`recruitment-kpi-icon ${s.iconBg} ${s.text}`}>{icon}</div>
        <span className={`text-[2.05rem] font-semibold tracking-[-0.01em] ${s.text}`}>{count}</span>
      </div>
      <p className={`recruitment-kpi-label opacity-80 ${s.text}`}>{label}</p>
    </div>
  );
};
