...
const JobsListView = ({ jobs, onJobClick, onDeleteJob, deletingJobId, statusEdit, onStatusChange, onRemarkChange, onSaveStatusEdit, onCancelStatusEdit }: JobsListViewProps) => (
  <div className="recruitment-table-shell">
    <table className="w-full text-left border-collapse">
      <thead className="recruitment-table-head-row sticky top-0 z-10">
        <tr>
          <th className="recruitment-table-head-cell w-10">
            <input type="checkbox" className="rounded border-gray-300" />
          </th>
          <th className="recruitment-table-head-cell">Job Title & ID</th>
          <th className="recruitment-table-head-cell">Client & Location</th>
          <th className="recruitment-table-head-cell">Status</th>
          <th className="recruitment-table-head-cell text-center">Indicators</th>
          <th className="recruitment-table-head-cell">Pipeline</th>
          <th className="recruitment-table-head-cell">Details</th>
        ...
        {jobs.length === 0 ? (
          <tr>
            <td colSpan={8} className="px-6 py-12 text-center text-sm font-medium text-slate-500">
              No jobs are available in this view yet.
            </td>
          </tr>
        ) : null}
        {jobs.map((job) => (
      ...
      {columns.map((col) => (
        <div key={col.id} className="min-w-[300px] flex-1 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            {jobs.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm font-medium text-slate-500 shadow-sm">
                No jobs to show in board view.
              </div>
            ) : null}
...
export default function JobsPage() {
  ...
  return (
    <div className="recruitment-page font-sans text-gray-900">
      <div className="custom-scrollbar overflow-y-auto">
          <div className="recruitment-page-shell max-w-[1440px]">
            <PageHeader
              title="My jobs"
              description="Jobs you create are loaded from the API. Seeded or other users' jobs are not listed here."
              meta={<CountPill>{jobs.length} jobs</CountPill>}
              actions={
                <>
                <div className="recruitment-segmented">
                  <button
                    ...
                    className={`recruitment-segmented-button ${view === 'list' ? 'recruitment-segmented-button-active' : ''}`}
                  >
                  ...
                  <button
                    ...
                    className={`recruitment-segmented-button ${view === 'board' ? 'recruitment-segmented-button-active' : ''}`}
                  >
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
              {STATS_CONFIG.map((statConfig, i) => {
                ...
                return (
                  <div key={i} className="recruitment-kpi-card group cursor-default">
                    <div className="mb-4 flex items-start justify-between">
                      <div className={`recruitment-kpi-icon ${statConfig.bg} ${statConfig.color} transition-transform group-hover:scale-110`}>
                        <StatIcon size={22} />
                      </div>
                      {loadingMetrics ? (
                        <div className="recruitment-kpi-trend bg-slate-100 text-slate-400">
                          Loading...
                        </div>
                      ) : (
                        <div className="recruitment-kpi-trend bg-slate-100 text-slate-500">
                          +12% <ChevronDown size={12} className="inline rotate-180" />
                        </div>
                      )}
                    </div>
                    <div className="space-y-1">
                      <p className="recruitment-kpi-value">{loadingMetrics ? '—' : value}</p>
                      <p className="recruitment-kpi-label">{statConfig.label}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="relative">
                    <button 
                      ...
                      className={`recruitment-secondary-button ${isFilterOpen ? 'border-blue-200 bg-blue-50 text-blue-700' : ''}`}
                    >
                      <Filter size={18} />
                      Filters
                  ...
                  <div className="recruitment-chip">
                  ...
                  <button className="text-xs font-black uppercase tracking-[0.14em] text-blue-600 hover:underline">Clear All</button>
                </div>
              ...
              <AnimatePresence>
                {isFilterOpen && (
                  ...
                  >
                    <div className="recruitment-surface grid grid-cols-1 gap-6 p-6 md:grid-cols-2 xl:grid-cols-4">
            ...
            >
              ...
              {loading ? (
                <div className="recruitment-surface p-8 text-center text-sm text-gray-500">
                  Loading jobs from API...
                </div>
              ) : jobs.length === 0 ? (
                <EmptyStateCard
                  title="No jobs available yet"
                  description="Create a new job to start filling roles, or adjust the visible filters if you expect to see existing jobs."
                  actions={
                    <>
                      <button type="button" onClick={() => setCreateJobDrawerOpen(true)} className="recruitment-primary-button">
                        <Plus size={15} className="shrink-0" />
                        Create Job
                      </button>
                      <button type="button" onClick={() => setIsFilterOpen(true)} className="recruitment-secondary-button">
