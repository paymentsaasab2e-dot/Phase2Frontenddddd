...
import { WidgetVisibilitySelector } from '../../components/dashboard/WidgetVisibilitySelector';
import {
  DASHBOARD_WIDGETS,
...
import { CountPill, EmptyStateCard, PageHeader } from '../../components/recruitment-ui';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    saveDashboardWidgetVisibility(widgetVisibility);
  }, [widgetVisibility]);
  ...
  const selectAllWidgets = useCallback(() => {
    setWidgetVisibility(createDefaultDashboardWidgetVisibility());
  }, []);

  const resetDefaultWidgets = useCallback(() => {
    setWidgetVisibility(createDefaultDashboardWidgetVisibility());
  }, []);
  ...
  const pipelineStages = useMemo(() => {
    ...
    const defs = [
      ...
      { label: 'Shortlisted', count: s.shortlist },
      { label: 'Sent to Client', count: s.submitted },
      { label: 'Selected', count: s.interviewing },
      { label: 'Offer Released', count: s.offered },
      { label: 'Joined', count: placementStats?.joined ?? s.hired },
    ];
    return defs.map((d, i) => ({
      ...d,
      color: PIPELINE_STYLES[i % PIPELINE_STYLES.length],
    }));
  }, [candidateStats, placementStats?.joined]);
  ...
  const selectedWidgetCount = useMemo(
    () => DASHBOARD_WIDGETS.filter((widget) => widgetVisibility[widget.key]).length,
    [widgetVisibility]
  );
  const hasVisibleWidgets = selectedWidgetCount > 0;
  const chartWidgetsVisibleCount =
    Number(widgetVisibility.openJobsByClient) + Number(widgetVisibility.jobStatus);
  const secondaryWidgetsVisibleCount =
    Number(widgetVisibility.myPendingTasks) + Number(widgetVisibility.teamPlacements);
  const lowerWidgetsVisibleCount =
    Number(widgetVisibility.alerts) +
    Number(widgetVisibility.recentActivity) +
    Number(widgetVisibility.revenueSnapshot);
  const chartGridClass =
    chartWidgetsVisibleCount > 1 ? 'grid grid-cols-1 gap-6 lg:grid-cols-12' : 'grid grid-cols-1 gap-6';
  const secondaryGridClass =
    secondaryWidgetsVisibleCount > 1 ? 'grid grid-cols-1 gap-6 lg:grid-cols-2' : 'grid grid-cols-1 gap-6';
  const lowerGridClass =
    lowerWidgetsVisibleCount === 3
      ? 'grid grid-cols-1 gap-6 pb-8 xl:grid-cols-3'
      : lowerWidgetsVisibleCount === 2
        ? 'grid grid-cols-1 gap-6 pb-8 md:grid-cols-2'
  ...
  return (
    <div className="recruitment-page-shell animate-in space-y-8 duration-500 fade-in">
      <PageHeader
        title="Agency Dashboard"
        description="Live recruitment metrics, operational widgets, and recent activity from your current frontend-connected data sources."
        meta={<CountPill>{selectedWidgetCount} active widgets</CountPill>}
        actions={
          <>
            <WidgetVisibilitySelector
              ...
              onResetDefaults={resetDefaultWidgets}
            />
            <button
              type="button"
              onClick={() => void load()}
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-6">
        {kpiCards.map((kpi, i) => (
          <Link
            ...
            className="recruitment-kpi-card group block outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-blue-500"
          >
            <div className="mb-4 flex items-start justify-between">
              <div className={`recruitment-kpi-icon ${kpi.bg} ${kpi.color} transition-transform group-hover:scale-110`}>
                <kpi.icon size={20} />
              </div>
              <div
                className={`recruitment-kpi-trend ${
            ...
            <div>
              <p className="recruitment-kpi-label">{kpi.title}</p>
              <p className="recruitment-kpi-value">{kpi.count}</p>
            </div>
          </Link>
        ))}
      </div>
      ...
      {widgetVisibility.candidatePipeline ? (
        <>
      {/* Pipeline */}
      <Link
        href="/pipeline"
            ...
            {pipelineStages.map((stage, i) => (
              <React.Fragment key={stage.label}>
                ...
                >
                  ...
                  <span className="text-xl font-semibold tracking-[-0.01em]">{formatCount(stage.count)}</span>
                </div>
      ...
      {chartWidgetsVisibleCount > 0 ? (
        <>
      {/* Charts */}
      <div className={chartGridClass}>
        {widgetVisibility.openJobsByClient ? (
        <Link
          href="/job"
          ...
          className={`block rounded-2xl border border-slate-200 bg-white p-6 shadow-sm outline-none ring-offset-2 transition-all hover:border-blue-200 hover:shadow-md focus-visible:ring-2 focus-visible:ring-blue-500 ${
            chartWidgetsVisibleCount > 1 ? 'lg:col-span-7' : ''
          }`}
        >
        ...
        >
          <div className="mb-6 flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-lg font-semibold text-slate-900">{aging != null ? `${aging} days` : '—'}</p>
            </div>
          </div>
          <div className="relative flex h-[250px] w-full items-center justify-center">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[1.75rem] font-semibold tracking-[-0.01em] text-slate-900">{jobPie.total}</span>
      ...
      {secondaryWidgetsVisibleCount > 0 ? (
        <div className={secondaryGridClass}>
        {widgetVisibility.myPendingTasks ? (
        <Link
          href="/Task&Activites"
        ...
        {widgetVisibility.teamPlacements ? (
        <Link
          href="/team"
      ...
      {widgetVisibility.todaysInterviews ? (
        <>
      {/* Interviews */}
      <Link
        href="/interviews"
      ...
      {lowerWidgetsVisibleCount > 0 ? (
        ...
        {widgetVisibility.revenueSnapshot ? (
        <Link
          href="/billing"
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                ...
                <p className="mt-1 text-[1.15rem] font-semibold text-slate-900">{revenueMain}</p>
              </div>
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Joining pipeline</p>
                <p className="mt-1 text-[1.15rem] font-semibold text-slate-900">{pendingApprox}</p>
              </div>
            </div>
            <div className="space-y-3">
        ...
        <EmptyStateCard
          ...
          description="Customize your agency dashboard to bring back the widgets you want to track."
          actions={
            <>
              <button
                type="button"
