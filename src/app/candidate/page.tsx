  ...
  return (
    <div className="recruitment-page">
      <div className="recruitment-page-shell max-w-[1440px]">
        <PageHeader
          title="My candidates"
          description="People you added plus applicants on jobs you created, with the same API-driven workflows preserved."
          meta={<CountPill>{filteredCandidates.length} shown</CountPill>}
          actions={
            <>
              <div className="recruitment-segmented">
                <button
                  onClick={() => setViewMode('list')}
                  className={`recruitment-segmented-button ${viewMode === 'list' ? 'recruitment-segmented-button-active' : ''}`}
                >
                  <List size={18} strokeWidth={viewMode === 'list' ? 2.4 : 2} />
                </button>
                <button
                  onClick={() => setViewMode('grid')}
                  className={`recruitment-segmented-button ${viewMode === 'grid' ? 'recruitment-segmented-button-active' : ''}`}
                >
                  <LayoutGrid size={18} strokeWidth={viewMode === 'grid' ? 2.4 : 2} />
                </button>
              </div>
              <button
              ...
              <button
                ...
                className={`recruitment-secondary-button ${isFilterOpen ? 'border-blue-200 bg-blue-50 text-blue-600' : ''}`}
              >
                <Filter size={16} />
                Filters
                {isFilterOpen && <span className="h-2 w-2 rounded-full bg-blue-600" />}
              </button>
            </>
          }
        />
        ...
        <div className="recruitment-table-shell overflow-hidden">
          {error && (
            <div className="m-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
              {error}
            </div>
          )}
          <div className="overflow-auto bg-white">
            {loading ? (
              <div className="px-6 py-12 text-center text-sm font-medium text-slate-500">Loading candidates from API...</div>
            ) : filteredCandidates.length === 0 ? (
              <div className="p-6">
                <EmptyStateCard
                  title="No candidates match this view"
                  description="Try another stage, adjust the current filters, or add a new candidate to keep the pipeline moving."
                  actions={
                    <>
                      <button
                        onClick={() => setIsFilterOpen(true)}
